...
void draw_zoom_gif(PRECISION x, PRECISION y, double zoom_coefficient, int num_of_zooms, unsigned int width, unsigned int height, unsigned int iteration_count);
// рисует gif, который будет приближаться к определенной точке
void draw_single_bmp(std::array<PRECISION, 4> size, unsigned int width, unsigned int height, unsigned int iteration_count);
// рисует одну bmp картинку
void draw_single_gif(std::array<PRECISION, 4> size, unsigned int width, unsigned int height, unsigned int iteration_count);
...
int main() {
    clock_t time = clock();
    std::array<PRECISION, 4> size; // лучше сделать с std::array
    size[0] = -2.0; // нижняя граница x
    size[1] = 2.0; // верхняя граница x
    size[2] = -1.125; // нижняя граница y
    size[3] = 1.125; // верхняя граница y
    unsigned int width = 960; // ширина изображения, которую я хочу получить
    unsigned int height = round(static_cast<PRECISION>(width) * (size[3] - size[2]) / (size[1] - size[0])); // высоту посчитаем по пропорции
    unsigned int iteration_count = 20000; // количество итераций для прорисовки
    //draw_zoom_gif(-0.777807810193171, 0.131645108003206, 0.9, 310, width, height, iteration_count);
